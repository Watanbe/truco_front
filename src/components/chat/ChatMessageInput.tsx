...
export default function ChatMessageInput(props: { onSendMessage: (message: string) => void }) {
	...
	return (
		<>
			<form className="flex items-center gap-3 p-3"
			style={{
				margin: "10px",
				borderRadius: "10px",
			...
			onSubmit={sendMessage}>
				...
				/>
				<button
					className="flex rounded-full bg-green-400 p-3 text-gray-50 shadow-sm focus-within:outline focus-within:bg-green-700 hover:bg-blue-600 disabled:bg-green-300"
					type="submit"
					disabled={!message}
				>
					<Icon>send</Icon>
